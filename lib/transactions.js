...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.SIKENd.height - transaction.height + 1;
  }
...
TxController.prototype.transformQtumTransaction = function(transaction) {
  ...
  if(blockHash) {
    ...
    self.node.getAddressHistory(address, options, function(err, result) {
      ...
      return async.eachSeries(txs, function (tx, callback) {
        ...
        if (err) {
            return self.common.handleErrors(new Error('Receip update error'), res);
        }
